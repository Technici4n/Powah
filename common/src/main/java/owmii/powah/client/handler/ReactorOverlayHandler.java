...
public class ReactorOverlayHandler {
    ...
    public static void onRenderLast(PoseStack matrix) {
        ...
        if (result instanceof BlockHitResult br) {
            ...
            buffer.vertex(matrix.last().pose(), pos.getX(), pos.getY(), pos.getZ() + 3).color(r, g, b, 1.0F).uv(0.0F, 1.0F).uv2(Render.MAX_LIGHT)
                    .endVertex();
            buffer.vertex(matrix.last().pose(), pos.getX() + 3, pos.getY(), pos.getZ() + 3).color(r, g, b, 1.0F).uv(1.0F, 1.0F).uv2(Render.MAX_LIGHT)
                    .endVertex();
            buffer.vertex(matrix.last().pose(), pos.getX() + 3, pos.getY(), pos.getZ()).color(r, g, b, 1.0F).uv(1.0F, 0.0F).uv2(Render.MAX_LIGHT)
                    .endVertex();
            buffer.vertex(matrix.last().pose(), pos.getX(), pos.getY(), pos.getZ()).color(r, g, b, 1.0F).uv(0.0F, 0.0F).uv2(Render.MAX_LIGHT)
                    .endVertex();
