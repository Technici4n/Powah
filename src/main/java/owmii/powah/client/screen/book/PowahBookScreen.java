...
import owmii.lib.client.compat.JEI;
...
public class PowahBookScreen extends ScreenBase {
    ...
    public PowahBookScreen(BookEntry entry, int index) {
    ...
    protected void init() {
        ...
        for (int i = 0; i < mainEntries.size(); i++) {
            ...
            this.catButtons[i] = new IconButton(this.x - 24, this.y + 20 + (i * 25), this.entry.getMain() == i ? 30 : 24, 24, 220, 0, 0, GUI_TEXTURE, button -> {
                this.mc.displayGuiScreen(new PowahBookScreen(entry.setMain(main), 0));
                refreshButtons();
        ...
        this.mainButtons[1] = new IconButton(this.x + this.w + 2, this.y + this.h - 32, 12, 23, 208, 58, 0, GUI_TEXTURE, button -> {
            ...
            this.mc.displayGuiScreen(new PowahBookScreen(this.entry, this.curPage).setPrevScreen(this.prevScreen));
            refreshButtons();
        }, this);
    ...
    public void open(BookEntry entry, int i) {
        this.mc.displayGuiScreen(
                new PowahBookScreen(entry, i).setPrevScreen(this));
    }

    @Nullable
    public PowahBookScreen getPrevScreen() {
        return this.prevScreen;
    }

    public PowahBookScreen setPrevScreen(@Nullable PowahBookScreen prevGui) {
