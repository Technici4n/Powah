...
public class ThermoCoolantCategory implements IRecipeCategory<ThermoCoolantCategory.Recipe> {
    public static final ResourceLocation GUI_BACK = new ResourceLocation(Powah.MOD_ID, "textures/gui/jei/misc.png");
    public static final ResourceLocation ID = new ResourceLocation(Powah.MOD_ID, "thermo.coolant");
        ...
        public static List<Recipe> getBucketRecipes(IIngredientManager ingredientManager) {
            ...
            allItemStacks.forEach(stack -> {
                if (stack.getItem() instanceof BucketItem && !(stack.getItem() instanceof FishBucketItem)) {
                    ...
                    if (PowahAPI.THERMO_COOLANTS.containsKey(fluid)) {
                        recipes.add(new Recipe(bucket, PowahAPI.getThermoCoolant(fluid)));
                    }
                }
            });

            List<Fluid> fluids = new ArrayList<>(PowahAPI.THERMO_COOLANTS.keySet());
